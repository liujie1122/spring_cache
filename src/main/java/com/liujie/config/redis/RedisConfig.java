...
public class RedisConfig extends CachingConfigurerSupport {
     ...
     * 当我们的数据存储到Redis的时候，我们的键（key）和值（value）都是通过Spring
     * 提供的Serializer序列化到数据库的。
...
// 所需自定义一个RedisTemplate，使用其他序列化方式
...
//        defaultCacheConfig.entryTtl(Duration.ofSeconds(30));
         ...
         */
        ParserConfig.getGlobalInstance().addAccept("com.liujie.server.bean.*");
        return cacheManager;
    }
    ...
    public RedisCacheConfiguration redisCacheConfiguration(){
        ...
        configuration = configuration.serializeValuesWith(RedisSerializationContext.SerializationPair.fromSerializer(fastJsonRedisSerializer)).entryTtl(Duration.ofDays(60*60*10));
